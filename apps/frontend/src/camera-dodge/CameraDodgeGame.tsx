...
import { Camera, CameraOff, Play, RotateCcw, ShieldAlert } from "lucide-react";
...
interface CameraDodgeGameProps {
  embedded?: boolean;
  paused?: boolean;
  restartKey?: number;
...
export function CameraDodgeGame({
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (state.status === "playing" && !externalPausedRef.current) {
  ...
  if (embedded) {
    return (
      <main className="camera-dodge-runtime">
        <section className="camera-dodge-stage">
          <canvas ref={canvasRef} className="camera-dodge-game-canvas" />
          {paused && game.status === "playing" && <div className="camera-dodge-pause-chip">Tạm dừng</div>}
          {game.status !== "playing" && (
            <button
              onClick={startGame}
              disabled={!cameraReady}
              className="camera-dodge-start-button"
        ...
        <aside className="camera-dodge-panel">
          <div className="camera-dodge-panel-heading">
            <span>
              <Camera size={19} />
              Camera
            </span>
            <button
              type="button"
              onClick={() => onCameraPreviewVisibleChange?.(!cameraPreviewVisible)}
              disabled={!onCameraPreviewVisibleChange}
            >
              {cameraPreviewVisible ? <CameraOff size={16} /> : <Camera size={16} />}
              {cameraPreviewVisible ? "Ẩn" : "Hiện"}
            </button>
          </div>

          <div className={cameraPreviewVisible ? "camera-dodge-preview" : "camera-dodge-preview is-collapsed"}>
            <video ref={videoRef} muted playsInline />
            <canvas ref={trackingCanvasRef} />
          </div>
          {!cameraPreviewVisible && <div className="camera-dodge-preview-hidden">Camera vẫn đang theo dõi. Hiện lại preview nếu cần căn giữa.</div>}

          {cameraError && <p className="camera-dodge-error is-danger">{cameraError}</p>}
          {poseError && <p className="camera-dodge-error">{poseError}</p>}

          <div className="camera-dodge-metrics">
            <div>
              <span>Tâm</span>
              <strong>{Math.round(poseCenter * 100)}%</strong>
            </div>
            <div>
              <span>Pose</span>
          ...
          <div className="camera-dodge-actions">
            <button type="button" onClick={enableCamera}>
              <Camera size={18} />
              {cameraReady ? "Đã bật" : "Bật camera"}
            </button>
            <button type="button" onClick={() => void loadPoseLandmarker()}>
              {poseStatus === "loading" ? "Đang tải" : "Tải pose"}
            </button>
            <button type="button" onClick={centerPoseTracking}>
              Căn giữa
            </button>
            <button
              type="button"
          ...
          <label className="camera-dodge-sensitivity">
            <span>Độ nhạy: {poseSensitivity.toFixed(1)}x</span>
            <input
              type="range"
              min="1"
              max="5"
        ...
        <section className="camera-dodge-score-panel">
          <div>
            <span>Thời gian</span>
          ...
          <div>
            <span>Kỷ lục</span>
            <strong>{Math.floor(game.bestScore)}</strong>
          </div>
          <div>
            <span>Combo</span>
          ...
          <div>
            <span>Pose X</span>
            <strong>{Math.round(trackingX * 100)}%</strong>
          </div>
        </section>
      </main>
    );
