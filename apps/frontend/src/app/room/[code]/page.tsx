...
export default function UserRoomPage() {
  ...
  const handleJoin = async () => {
    await joinRoom(roomCode, displayName.trim() || "Khách");
  };
  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-4 py-6 md:px-8">
      <h1 className="mb-4 text-2xl font-bold">Màn hình Người dùng</h1>
      <Link href="/karaoke" className="mb-3 inline-block text-xs text-slate-400 hover:text-cyan-300">← Quay lại Karaoke</Link>
      ...
      {role !== "guest" ? (
        <section className="max-w-md rounded-2xl border border-slate-700 bg-slate-900/60 p-4">
          <p className="mb-2 text-sm text-slate-300">Đang vào phòng <span className="font-bold text-cyan-200">{roomCode}</span></p>
          <input
        ...
        <section className="grid gap-4 lg:grid-cols-[1.2fr_1fr]">
          ...
          <aside className="rounded-2xl border border-slate-700 bg-slate-900/60 p-4">
            <div className="mb-2 flex items-center justify-between gap-2">
              <h2 className="text-lg font-semibold">Hàng đợi thời gian thực</h2>
              <button
            ...
            <p className="mb-1 mt-1 text-xs text-slate-400">Đang phát: {nowPlaying?.title ?? "Chưa có bài"}</p>
            <p className="mb-3 text-xs text-slate-500">Hàng đợi: {queue.length}/{maxQueueSize}</p>
