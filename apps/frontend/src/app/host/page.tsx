  ...
  return (
    <main className="mx-auto min-h-screen max-w-7xl px-4 py-6 md:px-8">
      <div className="space-y-4">
        <h1 className="text-2xl font-bold">Màn hình Chủ phòng</h1>
        <Link href="/karaoke" className="text-xs text-slate-400 hover:text-cyan-300">← Quay lại Karaoke</Link>
        ...
        {!roomCode ? (
          <section className="max-w-md rounded-2xl border border-slate-700 bg-slate-900/60 p-4">
            <label className="mb-2 block text-sm text-slate-300">Tên chủ phòng</label>
            <input
            ...
            <section className="lg:grid lg:grid-cols-[2fr_1fr] lg:gap-4">
              <div className="space-y-3 rounded-2xl border border-slate-700 bg-slate-900/60 p-4">
                ...
                <div className="rounded-xl border border-slate-700 bg-slate-950/70 p-3">
                  <p className="text-xs uppercase tracking-widest text-slate-400">Đang phát</p>
                  <p className="text-lg font-semibold text-slate-100">{nowPlaying?.title ?? "Chưa có bài đang phát"}</p>
                  <p className="text-sm text-slate-400">{nowPlaying?.addedByName ?? "Đang chờ hàng đợi"}</p>
                  <p className="mt-1 text-xs text-slate-500">Hàng đợi: {queue.length}/{maxQueueSize}</p>
                </div>
                <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-700 bg-slate-950/60 p-3">
                  <label className="text-xs text-slate-400">Giới hạn hàng đợi</label>
                  <input
              ...
              <aside className="mt-4 hidden rounded-2xl border border-slate-700 bg-slate-900/60 p-4 lg:mt-0 lg:block">
                <h2 className="mb-3 text-lg font-semibold">Hàng đợi</h2>
