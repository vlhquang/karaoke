...
export async function scrapeStockPrice(symbol: string, isRefresh: boolean = false): Promise<ScrapedPrice | null> {
    const headers = {
    ...
    for (const url of urls) {
        try {
            ...
            // Extraction strategy 4: Meta Description
            if (!priceValue) {
            ...
            console.error(`Scrape error for ${symbol} @ ${url}:`, error);
        }
    }
